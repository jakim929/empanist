...
Template.cropUploader.onRendered(function () {
	...
	if(!template.originalCanvas)
	{
		...
		// Added by JamesKim
		Session.set("previewPresent", false);
	...
	this.thumbnail_img.onload = function(e) {
		...
		thumbnail_ctx.drawImage(
			...
			// Added by JamesKim to deal with non-thumbnails -> Change back later with better thumbnail function
		...
		Session.set("previewPresent", true);
	}
});
...
Template.cropUploaderPlain.helpers({
	getPreviewPresent: function() {
		return Session.get("previewPresent")
	},
});
...
Template.cropUploaderCropper.onCreated(function () {
	...
	this.initCropper = function(template) {
		...
		//
		//*7*template.cropimage.cropper(options);
	}
});
	...
	Tracker.afterFlush(function () {
		...
		img.onload = function() {
			if(Meteor.isDevelopment) console.debug('img loaded');
				...
				if(Session.get("initializeCropper")){
